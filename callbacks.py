...
class ImageDisplayCallback(Callback):
    def __init__(self,
                 xy_train, xy_test, args,
                 modelDict, sampler,
                 **kwargs):
        self.x_train, self.y_train = xy_train
        self.x_test,  self.y_test  = xy_test

        self.image_count = args.batch_size

        self.args = args
        self.modelDict = modelDict
        consistent = True
        if consistent:
            assert self.image_count % args.y_label_count == 0
            rp = sampler(self.image_count // args.y_label_count, args.latent_dim)
            self.randomPoints = np.tile(rp, (args.y_label_count, 1))
        else:
            self.randomPoints = sampler(self.image_count, args.latent_dim)
        # labels = np.random.randint(args.y_label_count, size=(self.image_count, ))
        labels = np.arange(self.image_count, dtype=np.int32) * args.y_label_count // self.image_count
        self.randomLabels = keras.utils.to_categorical(labels)
    ...
    def on_epoch_end(self, epoch, logs):
        ...
        randomImages = self.modelDict.generator.predict([self.randomPoints, self.randomLabels], batch_size=self.image_count)
        assert self.image_count % self.args.y_label_count == 0
        vis.plotImages(randomImages, self.image_count // self.args.y_label_count, self.args.y_label_count, "{}/random-{}".format(self.args.outdir, epoch))
        vis.plotImages(randomImages, self.image_count // self.args.y_label_count, self.args.y_label_count, "{}/random".format(self.args.outdir))

        x_trainBatch = self.x_train[:self.image_count]
        y_trainBatch = self.y_train[:self.image_count]
        vis.displayReconstructed([x_trainBatch, y_trainBatch], self.args, self.modelDict, "{}/train-{}".format(self.args.outdir, epoch))
        vis.displayReconstructed([x_trainBatch, y_trainBatch], self.args, self.modelDict, "{}/train".format(self.args.outdir))

        x_testBatch = self.x_test[:self.image_count]
        y_testBatch = self.y_test[:self.image_count]
        vis.displayReconstructed([x_testBatch, y_testBatch], self.args, self.modelDict, "{}/test-{}".format(self.args.outdir, epoch))
        vis.displayReconstructed([x_testBatch, y_testBatch], self.args, self.modelDict, "{}/test".format(self.args.outdir))
